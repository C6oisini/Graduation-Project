...
class QwenVLvMFWrapper:
    ...
    def _setup_hooks(self):
        ...
        if self.perturb_visual:
            ...
            if visual_encoder is not None:
                ...
                print(f"[vMF] 已注册视觉编码器 hook: {type(visual_encoder).__name__}")
            else:
                print("[vMF] 警告: 未找到视觉编码器模块")
        ...
        if self.perturb_text:
            ...
            if text_embedding is not None:
                ...
                self._hooks.append(hook)
                print(f"[vMF] 已注册文本嵌入 hook: {type(text_embedding).__name__}")
            else:
                print("[vMF] 警告: 未找到文本嵌入模块")

        print(f"[vMF] 配置: epsilon={self.epsilon}, beta={self.beta}, alpha={self.alpha}")

    def _find_visual_encoder(self):
        """查找视觉编码器的最终投影层（输出 4096 维度）"""
        # 优先查找 merger.linear_fc2（最终投影层，输出 4096 维度）
        possible_paths = [
            'model.visual.merger.linear_fc2',  # Qwen3-VL merger 最后一层
            'visual.merger.linear_fc2',
            'model.visual.merger',             # Qwen3-VL merger
            'visual.merger',
            'visual',
            'vision_model',
            'vision_tower',
        ...
        for name, module in self.model.named_modules():
            if 'merger' in name.lower() and 'linear_fc2' in name.lower() and 'deepstack' not in name.lower():
                return module
    ...
    def _find_text_embedding(self):
        """查找文本 embedding 模块（输出 4096 维度）"""
        possible_paths = [
            'model.language_model.embed_tokens',  # Qwen3-VL
            'model.embed_tokens',
            'embed_tokens',
    ...
    def _visual_hook(self, module, input, output):
        ...
        if isinstance(output, torch.Tensor):
            ...
            original_std = output.std().item()
            shape = output.shape
        else:
            original_norm = original_mean = original_std = 0
            shape = "N/A"

        result = self._apply_perturbation(output, 'visual')

        # 计算扰动后的统计信息
        if isinstance(result, torch.Tensor):
            perturbed_norm = torch.norm(result).item()
            perturbed_mean = result.mean().item()
            perturbed_std = result.std().item()

            # 计算角度偏差
            cos_sim = torch.nn.functional.cosine_similarity(
                output.view(-1), result.view(-1), dim=0
            ).item()
            angle_deg = torch.acos(torch.tensor(cos_sim)).item() * 180 / 3.14159

            print(f"[vMF] 视觉扰动 | shape={shape} | epsilon={self.epsilon}")
            print(f"      原始: norm={original_norm:.2f}, mean={original_mean:.4f}, std={original_std:.4f}")
            print(f"      扰动: norm={perturbed_norm:.2f}, mean={perturbed_mean:.4f}, std={perturbed_std:.4f}")
            print(f"      角度偏差: {angle_deg:.2f}°")
    ...
    def _text_hook(self, module, input, output):
        ...
        result = self._apply_perturbation(output, 'text')
        ...
        if isinstance(result, torch.Tensor):
            ...
            print(f"[vMF] 文本扰动 | shape={shape} | epsilon={self.epsilon * self.alpha}")
            print(f"      原始: norm={original_norm:.2f}, mean={original_mean:.4f}, std={original_std:.4f}")
            print(f"      扰动: norm={perturbed_norm:.2f}, mean={perturbed_mean:.4f}, std={perturbed_std:.4f}")
            print(f"      角度偏差: {angle_deg:.2f}°")
